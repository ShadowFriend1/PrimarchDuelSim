...
class Primarch:
    ...
    def servo_fight(self, hit_mod, wound_mod, e_ws, e_t, dorn):
        hits = self.hit(hit_mod, e_ws, 1)
        return self.wound(hit_mod, 8, wound_mod, e_t, False, False, dorn, 2, False), False, False, False, False
    ...
    def wound(self, hits: int, strength, wound_mod, e_t, fp_t, fp_i, dorn, ap, fp_w):
        ...
        if not (fp_i & fp_w):
            for N in range(hits):
                roll = random.randint(1, 6)
                if (roll >= self.murderous) or ((roll >= wound_c) & (strength >= (e_t * 2))):
